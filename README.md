...
前四章从模型协议进入 Agent Loop 与工具执行；第五、六章继续追踪 Context 怎样组成、信息怎样跨越多轮；第七章解释 Host 如何通过 MCP 发现并调用外部能力；第八、九章说明工作方法、扩展代码与分发单元怎样进入 Harness；第十、十一章进入多步骤任务，先划清 Workflow 与 Agent 的控制权，再学习串行、路由、并行、动态 worker 与评价循环等组合结构。
